...
class Memory(imemSizeInBytes: Int = 16384) extends Module {
  ...
  val dram_i_wen   = Wire(Bool())
  val dram_i_addr  = Wire(UInt((WORD_LEN-CACHE_LINE_BITS).W))
  val dram_i_wdata = Wire(UInt(CACHE_LINE_LEN.W))
    ...
    is (DramState.Ready) {
      when (io.dramPort.init_calib_complete && !io.dramPort.busy) {
        ...
        when (dram_i_wen) {
          ...
          io.dramPort.wdata := dram_i_wdata.asTypeOf(new LineBundle()).line_l
          io.dramPort.wmask := 0.U
          reg_dram_addr := dram_i_addr
          reg_dram_wdata := dram_i_wdata.asTypeOf(new LineBundle()).line_u
