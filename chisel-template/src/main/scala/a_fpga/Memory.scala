...
class Memory(dataMemoryPath: String = null, imemSizeInBytes: Int = 16384, dmemSizeInBytes: Int = 16384) extends Module {
  ...
  val read_req_count = RegInit(0.U(8.W))
  val read_addr = RegInit(1.U(WORD_LEN.W))
  val read_req = io.dmem.ren && io.dmem.rready && ((io.dmem.raddr & ~3.U) =/= read_addr)
  read_req_count := MuxCase(read_req_count, Seq(
    (read_req && !io.dramPort.rdata_valid) -> (read_req_count + 1.U),
    (!read_req && io.dramPort.rdata_valid) -> (read_req_count - 1.U),
  ))
  when (read_req) {
    read_addr := io.dmem.raddr & ~3.U
  }.elsewhen (io.dmem.rvalid) {
    read_addr := 1.U(WORD_LEN.W)
  }

  io.dramPort.ren := read_req
  ...
  io.dmem.rvalid := io.dramPort.rdata_valid && (read_req_count === 1.U)
