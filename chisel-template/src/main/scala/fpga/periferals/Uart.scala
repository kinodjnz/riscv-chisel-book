...
class UartTx(numberOfBits: Int, baudDivider: Int) extends Module {
  ...
  val rateCounter = RegInit((baudDivider - 1).U(log2Ceil(baudDivider).W))
  val bitCounter = RegInit(0.U(log2Ceil(numberOfBits + 1).W))
  val bits = RegInit(VecInit((0 until (numberOfBits + 1)).map(_ => true.B)))

  io.tx := bits(0)
  io.in.ready := bitCounter === 0.U

  when (io.in.valid && io.in.ready) {
    bits := Cat(io.in.bits, 0.U).asBools   // (STOP(1)), DATA, START(0)
  ...
  when (rateCounter === 0.U) {
    // Shift out from LSB
    (0 until numberOfBits).foreach(i => bits(i) := bits(i + 1))
    bits(numberOfBits) := true.B
    when (bitCounter > 0.U) {
